				...
				<form action="updateData.php?keyword=<?= $keyword ?>&id=<?php echo $user['id'] ?>&page=<?php echo $page ?>" method="POST" role="form">
					...
					<div class="form-group">
						...
						<select name="class" id="inputClass" class="form-control">
							<?php 
								while ($rowClass = $successClasses->fetch_assoc())
								{
							?>		
							<option value="<?= $rowClass['class_id'] ?>" <?php echo ($user['class_id'] == $rowClass['class_id']) ? "selected" :""  ?>><?= $rowClass['classname'] ?></option>
							<?php
								}
							?>   
						</select>
					...
					<div class="form-group">
						...
						<select name="role" id="inputClass" class="form-control">
								<option value="1" <?php echo $user['role'] == 1 ? "selected" : ""  ?>>Admin</option>  
								<option value="0" <?php echo $user['role'] == 0 ? "selected" : ""  ?>>Member</option>
						</select>
					</div>
